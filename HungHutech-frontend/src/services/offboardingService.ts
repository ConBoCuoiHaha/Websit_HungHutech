...
class OffboardingService {
  ...
  ): Promise<PaginatedResponse<OffboardingRequest>> {
    const response = await api.get<PaginatedResponse<OffboardingRequest>>(
      this.BASE_URL,
      {params},
    );
  ...
  async getById(id: string): Promise<OffboardingRequest> {
    const response = await api.get<OffboardingRequest>(
      `${this.BASE_URL}/${id}`,
    );
    return response.data;
  }
  ...
  async updateStatus(
    ...
    const response = await api.patch<OffboardingRequest>(
  ...
  async getUpcoming(params?: {
    ...
    const response = await api.get<{data: OffboardingRequest[]}>(
      `${this.BASE_URL}/upcoming`,
      {
        params,
      },
    );
