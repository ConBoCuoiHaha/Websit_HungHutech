<template>
  <div v-loading="loading" class="orangehrm-memberships">
    ...
    <el-empty
      v-else
      description="Chưa có thông tin thành viên tổ chức"
      :image-size="100"
    />
    ...
    >
      ...
      >
        <el-form-item label="Tổ chức/Hội" prop="to_chuc" required>
          <el-input
            v-model="form.to_chuc"
            placeholder="Tên tổ chức, hội, hiệp hội"
          />
        </el-form-item>

        <el-form-item label="Loại thành viên" prop="loai_thanh_vien" required>
          <el-select
            v-model="form.loai_thanh_vien"
        ...
        <el-form-item label="Mã thành viên" prop="ma_thanh_vien">
          <el-input
            v-model="form.ma_thanh_vien"
            placeholder="Mã số thành viên"
          />
        </el-form-item>
...
const handleEdit = (index: number) => {
  ...
  form.phi_thanh_vien = Number(
    membership.phi_thanh_vien?.$numberDecimal || membership.phi_thanh_vien || 0,
  );
...
const handleDelete = async (index: number) => {
  try {
    ...
    if (err !== 'cancel') {
      ElMessage.error(
        err.response?.data?.msg || 'Không thể xóa thành viên tổ chức',
      );
    }
  } finally {
    saving.value = false;
...
const handleSave = async () => {
  ...
  try {
    ...
    ElMessage.success(
      editIndex.value === -1
        ? 'Thêm thành viên tổ chức thành công'
        : 'Cập nhật thành viên tổ chức thành công',
    );
    dialogVisible.value = false;
    emit('reload');
  } catch (err: any) {
    ElMessage.error(
      err.response?.data?.msg || 'Không thể lưu thành viên tổ chức',
    );
  } finally {
    saving.value = false;
  }
